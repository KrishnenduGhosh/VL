      ...
      for d in data:
        for i in range(0,len(d['off'])):
          ...
          for j in range(0,len(docs)):
            ...
            sim = scores[j] #
            rank = (j+1) #
            file1 = open(dir_out + "/RER.txt","a")
            file1.write(qid + " > " + qid + "_" + docno + " " + str(rank) + " " + str(sim) + " RER")
            file1.close()
    with open(dir_in3 + t +'.json') as json_file:
      data = json.load(json_file)
      for d in data:
        for i in range(0,len(d['off'])):
          qid = d['off'][i] #
          ...
          for j in range(0,len(docs)):
            ...
            sim = scores[j] #
            rank = (j+1) #
            file1 = open(dir_out + "/GS.txt","a")
            file1.write(qid + " > " + qid + "_" + docno + " " + str(rank) + " " + str(sim) + " GS")
            file1.close()
