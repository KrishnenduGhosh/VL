...
def topic(t):
    ...
    out_dir_name1="./Annotation/"+t
#    print(t)
    ctr = 0
    for lecture in sorted(os.listdir(dir_name)):
        #print(lecture)
        for filename in sorted(os.listdir(os.path.join(dir_name, lecture))):
            if not os.path.exists(out_dir_name1):
                os.makedirs(out_dir_name1)
            file_path="./"+dir_name+"/"+lecture+"/"+filename
            file_opath="./"+out_dir_name1+"/"+lecture
#            print("file_path: ",file_path)
            if not os.path.exists(file_opath):
                os.makedirs(file_opath)
