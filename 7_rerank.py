...
#      print(i)
      rscore[i] = score[i] + (10 * aws) + (10 * nm) + (10 * pc) + (10 * dc) + (1 * tfidf) ## test with different weights
  ...
  for t in topics:
    ...
    with open(dir_in + t +'.json') as json_file:
      ...
      for d in data:
        ...
        wdata.append({'id': d['id'],'text': d['text'],'topics': d['topics'],'mentions': d['mentions'],'off': d['off'],'ret': d['ret'],'score': d['score'],'reranked': re})
...
#        qid = d['id'] + ":"
 ...
 #         print("qid: ",qid)
          ...
          for j in range(0,len(docs)):
            ...
            sim = scores[j] #
            rank = (j+1) #
            rtdata.append(qid + " > " + qid + ":" + docno + " " + str(rank) + " " + str(sim) + " RR")
